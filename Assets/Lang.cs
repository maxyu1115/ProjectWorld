...
public class Lang
{
    ...
    */
	...
	public void setLanguage (string name, string language){
		...
		//xml.Load(path);
		var temp = Resources.Load<TextAsset>(name);
		XmlDocument xml = new XmlDocument();
		xml.LoadXml (temp.text);
		...
		if (element != null) {
			...
			while (elemEnum.MoveNext()) {
				...
				Strings.Add(xmlItem.GetAttribute("name"), xmlItem.GetAttribute("_text"));
			}
			Debug.Log ("Language package maybe success?");
		} else {
