...
//					labelsShadowMaterial.renderQueue--;
	...
	#region Public API area
		...
		public void ToggleCountrySurface (int countryIndex, bool visible, Color color, Texture2D texture, Vector2 textureScale, Vector2 textureOffset, float textureRotation, bool applyTextureToAllRegions)
		{
			...
			if (applyTextureToAllRegions) {
				for (int i = 0; i < country.regions.Count; i++) {
					ToggleCountryRegionSurface (countryIndex, i, visible, color, texture, textureScale, textureOffset, textureRotation, applyTextureToAllRegions);
				}
			}else{
		...
		//MAX!
		/// <summary>
		/// Refreshs the lists of country neighbors.
		/// </summary>
		void refreshCountryNeighbors(){
			refreshRegionCountryIndices ();

			Country c;
			Region r;
			for (int i = 0; i < countries.Length; i++) {
				c = countries [i];
				c.countryNeighborIndices = new List<int> ();
				for (int j = 0; j < c.regions.Count; j++) {
					r = c.regions [j];
					for (int k = 0; k < r.neighbours.Count; k++) {
						if (!c.countryNeighborIndices.Contains (r.neighbours [k].countryIndex)) {
							c.countryNeighborIndices.Add (r.neighbours [k].countryIndex);
						}
					}
				}
			}
		...
		//MAX!
		/// <summary>
		/// Refreshs the region country indices.
		/// </summary>
		void refreshRegionCountryIndices(){
			Country c;
			Region r;
			for (int i = 0; i < countries.Length; i++) {
				c = countries [i];
