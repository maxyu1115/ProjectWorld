...
//				mr.useLightProbes = false;
		...
		GameObject GenerateCountryRegionSurface (int countryIndex, int regionIndex, Material material, Vector2 textureScale, Vector2 textureOffset, float textureRotation, bool drawOutline, bool applyTextureToAllRegions = true)
		{
			...
			if (preloadTriangulators) {
				surfIndices = new int[countryTriangulated [countryIndex] [regionIndex].Length];
				Array.Copy (countryTriangulated [countryIndex] [regionIndex], surfIndices, countryTriangulated [countryIndex] [regionIndex].Length);
			} else {
				surfIndices = Triangulator.GetPoints (region.points);
			...
			if (_highlightAllCountryRegions) {
				...
				for (int r = 0; r < crCount; r++) {
					...
					if (otherRegion != region) {
						...
						//MAX!
						if (preloadTriangulators) {
							otherSurfIndices = new int[countryTriangulated [countryIndex] [r].Length];
							Array.Copy (countryTriangulated [countryIndex] [r], otherSurfIndices, countryTriangulated [countryIndex] [r].Length);
						} else {
							otherSurfIndices = Triangulator.GetPoints (otherRegion.points);
						}
		...
		{
			// Proxy function which now uses the packed method to reduce draw calls
