...
namespace WPMF
{
	...
	public partial class WorldMap2D_EditorInspector : Editor
	{
		...
		GUIStyle blackStyle, labelsStyle, areaStyle, attribHeaderStyle, warningLabelStyle, editorCaptionLabelStyle;
		...
		void ShowEntitySelectors ()
		{
			...
			if (_editor.countryIndex >= 0) {
				...
				//Data Editor
				EditorGUILayout.BeginHorizontal ();
				GUILayout.Label ("Data", GUILayout.Width (120));
				EditorGUILayout.EndHorizontal ();
				...
				EditorGUILayout.BeginHorizontal ();
				GUILayout.Label ("Natural Disaster Buffs", GUILayout.Width (120));
				EditorGUILayout.EndHorizontal ();
		...
		void ProcessOperationMode ()
		{
			...
			case OPERATION_MODE.SELECTION:
				...
				if (mouseDown) {
					...
					//MAX!
					if (!_editor.map.showCities || !_editor.CitySelectByScreenClick (ray)) {
		...
		bool SaveChanges ()
		{
			...
			if (!_editor.countryChanges && !_editor.provinceChanges && !_editor.cityChanges && !_editor.mountPointChanges && !_editor.dataChanges && !_editor.ndChanges)
				return false;
			...
			if (_editor.mountPointChanges && _map.showCities) {
			...
			//MAX!
			//_editor.triangulatorChanges = _editor.countryChanges || _editor.provinceChanges || _editor.cityChanges;
